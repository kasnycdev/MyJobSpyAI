...
from myjobspyai.exceptions import ConfigurationError, LLMError, ProviderError
from myjobspyai.analysis.models import AnalyzedJob, ResumeData
...
async def load_and_extract_resume_async(
    ...
    try:
        ...
        analyzer_config = config.dict() if hasattr(config, 'dict') else (config or {})
        resume_analyzer = ResumeAnalyzer(config=analyzer_config)
        logger.info(f"Analyzing resume with model: {resume_analyzer.model_name}")
    except Exception as e:
    ...
    try:
        with tracer.start_as_current_span("extract_resume_data_llm"):
            ...
            structured_resume_data = await resume_analyzer.analyze(resume_text)
