...
async def load_and_extract_resume_async(
    ...
    try:
        ...
        log.error(
            f"[bold red]Failed to parse resume text from {resume_path}: "
            f"{e}",
            exc_info=True,
        )
        return None
    if not resume_text:
        log.error("[bold red]Parsed resume text is empty.[/bold red]")
        return None
...
async def analyze_jobs_async(
    ...
    if RICH_AVAILABLE:
        progress_context = Progress(
            ...
            TextColumn("[progress.completed]{task.completed} of {task.total} jobs"),
            TimeElapsedColumn(),
        )
    else:
        log.warning("[yellow]Rich library not found, progress bar disabled.[/yellow]")

    results_or_exceptions = []
    if progress_context:
        with progress_context as progress:
            analysis_task_tracker = progress.add_task(
                "[cyan]Analyzing Jobs...", total=len(tasks)
            )
    ...
    for i, result_or_exc in enumerate(results_or_exceptions):
        ...
        if isinstance(result_or_exc, Exception):
            log.warning(
                ...
                f"[yellow]{result_or_exc}",
                exc_info=False,
            )
            log.debug("Exc details:", exc_info=result_or_exc)
    ...
    log.info(
        f"ASYNC analysis complete. Processed results for {len(analyzed_results)} jobs."
    )
...
async def main_async():
    ...
    if not structured_resume:
        ...
        return
    log.info(f"Loading jobs from Parquet file: {args.jobs}")
