...
def parse_salary(
    salary_text: Optional[str], target_currency: str = "USD"
    ...
    if range_match := re.search(r"(\d+)\s*[-–to]+\s*(\d+)", text):
        try:
            ...
            log.warning(
                f"Found range pattern but failed to convert numbers in: '{salary_text}'"
            )
    ...
    if up_to_match := re.search(
        ...
        try:
            ...
            log.warning(
                f"Found 'up to' pattern but failed to convert number in: '{salary_text}'"
            )
    ...
    if min_match := re.search(
        ...
        try:
            ...
            log.warning(
                f"Found 'min' pattern but failed to convert number in: '{salary_text}'"
            )
    ...
    if len(plausible_salaries) == 1:
        ...
        log.warning(
            f"Ambiguous salary - multiple numbers found in '{salary_text}': {plausible_salaries}. Taking min/max."
        )
