...
import traceback
import time # For Gemini retry delay
        ...
        for attempt in range(max_retries + 1): # +1 to make max_retries the number of retries, so total attempts = max_retries + 1
            try:
                ...
                if content is None:
                    ...
                    try:
                        # Since JSON mode is not explicitly requested/guaranteed,
                        # we might need to strip markdown fences if the model adds them.
                        ...
                        result = json.loads(content_strip) # Try parsing the potentially cleaned content
